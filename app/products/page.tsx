    ...
    <div>
      <PageHeader title="Products" subtitle="Manage stock, prices, and reorder levels">
        <input
          ...
          className="w-52 rounded-lg border border-slate-300 px-3 py-2 text-sm outline-none transition-colors duration-200 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-100"
        />
        <button
          onClick={openAdd}
                ...
                {products.map((p) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-3 py-3 text-right tabular-nums text-slate-500">{naira(p.cost_price)}</td>
                      <td className="px-3 py-3 text-right font-semibold tabular-nums text-slate-800">{naira(p.selling_price)}</td>
                      <td className={`px-3 py-3 text-right font-semibold tabular-nums ${low ? "text-amber-600" : "text-slate-700"}`}>
                        {p.quantity_in_stock}
                      </td>
      ...
      <Modal open={showAdd || editing !== null} onClose={() => { setShowAdd(false); setEditing(null); }} title={editing ? "Edit Product" : "Add Product"}>
        <div className="space-y-4">
          ...
          <div className="flex justify-end gap-2 pt-2">
            <button onClick={() => { setShowAdd(false); setEditing(null); }} className="rounded-lg px-4 py-2 text-sm font-semibold text-slate-500 transition-colors duration-200 hover:bg-slate-100">
              Cancel
            </button>
            <button
              onClick={save}
