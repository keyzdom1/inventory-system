  ...
  return (
    <div>
      ...
      <div className="mt-8 grid gap-6 lg:grid-cols-5">
        <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-700 dark:bg-slate-800 lg:col-span-3">
          <div className="mb-4 flex items-center justify-between">
            <h2 className="text-sm font-bold tracking-wide text-slate-500 uppercase dark:text-slate-400">Revenue This Month</h2>
            <Link href="/products" className="text-xs font-semibold text-indigo-600 hover:text-indigo-800 dark:text-indigo-400 dark:hover:text-indigo-300">
              View products →
            </Link>
          </div>
          {!monthly && !error ? (
        ...
        <section className="rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-slate-700 dark:bg-slate-800 lg:col-span-2">
          <div className="border-b border-slate-100 px-5 py-4 dark:border-slate-700">
            <h2 className="text-sm font-bold tracking-wide text-slate-500 uppercase dark:text-slate-400">Sales This Month</h2>
          </div>
          {!monthly && !error ? (
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {monthly.slice(0, 10).map((r, i) => (
                  <tr key={r.product_id} className={i % 2 === 0 ? "" : "bg-slate-50/60 dark:bg-slate-700/30"}>
                    <td className="max-w-40 truncate px-5 py-2.5 font-medium text-slate-700 dark:text-slate-300">{r.product_name}</td>
